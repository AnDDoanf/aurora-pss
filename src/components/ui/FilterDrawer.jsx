  ...
  return (
    <div className="space-y-4 bg-slate-900 p-4 rounded-lg shadow-sm">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder={t('common.filterCatalog')}
        ...
        <div className="flex w-full flex-wrap items-stretch gap-2 sm:w-auto sm:items-center sm:gap-3">
          {rarities && (
            <select
              ...
              className="min-w-0 flex-1 bg-slate-950 text-xs text-slate-100 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-indigo-500 font-medium sm:flex-none"
            >
          ...
          {onReset && (
            <button
              onClick={onReset}
              className="flex min-h-9 items-center justify-center space-x-1 px-2 py-1.5 text-xs font-medium text-slate-400 transition-colors hover:text-indigo-400"
            >
      ...
      <div className="flex items-center justify-between text-xs text-slate-400 pt-2 border-t border-slate-800/40">
        <span>{t('common.resultsFound', { count: totalResults })}</span>
      </div>
    </div>
  );
}
