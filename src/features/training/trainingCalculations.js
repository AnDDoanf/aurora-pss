...
export function calculateTrainingPossibilities(program, capacity, training, fatigue, targetStat, quality = 'regular') {
  ...
  const rows = TRAINING_STATS.map((stat) => {
    ...
    const requiredEffect = divisor > 0
      ? Math.floor((1 / divisor) * 10000) / 10000
    ...
    if (isMainStat && fatigueValue === 100) {
      ...
      && fatigueValue <= 50
      && requiredEffect > 0
      && effect / requiredEffect > 1.5
    ) {
      // This threshold branch intentionally ignores the 1/2 fatigue multiplier,
      // matching the reference tool's reverse-engineered game behavior.
